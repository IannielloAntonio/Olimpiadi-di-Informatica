...
Si osserva che la relazione~\eqref{eq:ricorrenza_base} coincide con la successione di Fibonacci, poiché ogni termine è ottenuto come somma dei due precedenti.
...
Come detto, ogni chiamata ricorsiva genera due ulteriori chiamate, dando origine a un albero di ricorsione binario. Di conseguenza, la complessità complessiva dell’algoritmo può essere stimata come \(O(2^N)\).
...
int main(){
	...
	int64_t ans;    // Variabile che conterra' il risultato finale
...
\begin{itemize}
	...
	\item Gli elementi del vector sono inizializzati a $-1$ perché, all'inizio, nessuno di essi era stato ancora calcolato.
\end{itemize}
...
\subsection{Discesa Massima}
\href{https://training.olinfo.it/task/discesa}{Link al problema} 
...
\includepdf[pages=-]{Problemi svolti/6 - Programmazione Dinamica/testo.pdf}
