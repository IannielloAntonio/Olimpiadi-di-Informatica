...
\label{problema:pattern-algoritmici:sliding-window:distinct-values-subarray}

\href{https://cses.fi/problemset/task/3420}{Link al problema}
...
\noindent \texttt{Explanation:} The subarrays are $[1]$ (two times), $[2]$, $[3]$, $[1,2]$, $[1,3]$, $[2,1]$ and $[2,1,3]$.
...
Utilizziamo la tecnica della \emph{sliding window} (due puntatori):
\begin{itemize}
	\item $left$ = inizio della finestra
	\item $right$ = fine della finestra
\end{itemize}
...
\begin{enumerate}
	\item Aggiungiamo $X[right]$ alla finestra.
	\item Se $X[right]$ compare più di una volta, spostiamo $left$ verso destra finché la finestra torna ad avere tutti elementi distinti.
\end{enumerate}

A questo punto la finestra $[left, right]$ contiene solo elementi distinti.  
Tutti i sottoarray che terminano in $right$ e partono tra $left$ e $right$ sono distinti e validi.  
...
Sommiamo questo valore al risultato totale.  
In questo modo ogni sottoarray viene contato esattamente una volta, quando il suo estremo destro coincide con $right$.
...
\begin{lstlisting}
	...
	int main(){
		int64_t N;
		cin >> N; // Legge la dimensione dell'array
		
		vector<int64_t> X(N);
		
		// Legge gli N elementi dell'array
		for(int i = 0; i < N; i++){
			cin >> X[i];
		}
		
		int left = 0;           // Puntatore sinistro della finestra (sliding window)
		int64_t count = 0;     // Contatore totale dei sottoarray validi
		map<int64_t,int64_t> m; // Mappa che tiene il conteggio delle frequenze degli elementi nella finestra
		
		// Il puntatore right scorre l'array da sinistra a destra
		for(int right = 0; right < N; right++){
			m[X[right]]++; // Inserisce X[right] nella finestra (incrementa la sua frequenza)
			
			// Se X[right] è già presente (frequenza > 1), restringiamo la finestra da sinistra
			while(m[X[right]] > 1){
				m[X[left]]--; // Rimuove X[left] dalla finestra (decrementa la sua frequenza)
				left++;       // Sposta il puntatore sinistro verso destra
			}
			
			// A questo punto la finestra [left, right] contiene solo elementi distinti
			// Tutti i sottoarray che finiscono in 'right' e partono tra left e right sono validi
			count += right - left + 1;
		}
		
		// Stampa il numero totale di sottoarray con elementi distinti
		cout << count << "\n";
	}
...
\label{problema:pattern-algoritmici:sliding-window:playlist}

\href{https://cses.fi/problemset/task/1141}{Link al problema}
...
\medskip
\noindent The next line has $n$ integers $k_1,k_2,\ldots,k_n$: the id number of each song.
...
Si considera la sequenza di $n$ canzoni come un array $K$ di lunghezza $n$, in cui ogni elemento rappresenta l'identificativo della canzone corrispondente.  
L'obiettivo è determinare la massima lunghezza di un segmento consecutivo di canzoni in cui ciascun elemento compare una sola volta.

Si utilizza la tecnica della \emph{sliding window} (finestra scorrevole) per esaminare in modo efficiente tutti i segmenti consecutivi possibili.  
...
\begin{itemize}
	\item Si estende la finestra incrementando l'indice $right$ e aggiornando una struttura dati che memorizza il numero di occorrenze di ciascuna canzone all'interno della finestra.
	\item Se l'inserimento di una nuova canzone provoca una ripetizione, la finestra non rispetta più il vincolo di unicità; in tal caso, si riduce la finestra incrementando l'indice $left$ finché la condizione di unicità non viene nuovamente soddisfatta.
	\item Ogni volta che la finestra rispetta il vincolo richiesto, si aggiorna la lunghezza massima del segmento trovato.
\end{itemize}

\medskip
In questo modo si esplorano tutti i segmenti consecutivi che non contengono ripetizioni senza dover considerare tutte le possibili sotto-sequenze.  
La lunghezza massima individuata durante il processo rappresenta la soluzione del problema.
...
int main(){
	...
	map<int, int> m;    // Mappa che tiene trac6cia del numero di occorrenze
...
\bigskip
Per applicare la tecnica della \texttt{bitmask} si procede nel seguente modo:

\begin{enumerate}
	\item Si costruisce un vettore di valori booleani (oppure binari) che rappresenta una configurazione.  
	Ogni posizione del vettore assume valore $0$ oppure $1$ e indica l’appartenenza dell’elemento corrispondente a uno dei due gruppi.
	
	\item A partire dalla configurazione definita dalla bitmask, si esegue il calcolo richiesto sui due gruppi ottenuti.
	
	\item Si incrementa di una unità il numero binario rappresentato dal vettore, generando così una nuova configurazione e una nuova suddivisione degli elementi in gruppi.
	
	\item Il procedimento viene ripetuto fino a esaurire tutte le possibili configurazioni.
\end{enumerate}
...
\begin{itemize}
	\item La prima riga contiene un intero \( N \) (\(1 \le N \le 20\)): il numero di valori.
	\item La seconda riga contiene \( N \) interi \( a_1, a_2, \dots, a_N \) (\(1 \le a_i \le 10^6\)), separati da spazio: i valori dell’insieme.
\end{itemize}
...
\[
1 \le N \le 20, \quad 1 \le a_i \le 10^6
\]

\bigskip
\noindent \textbf{Esempio di input}
...
\noindent \textbf{Esempio di output}
...
\bigskip \textbf{Implementazione C++:}
...
{
	int N;
	cin >> N;  // legge il numero di elementi dell'array
	
	vector<int64_t> a(N);  // array degli elementi
	for(int i=0; i<N; i++){
		cin >> a[i];  // legge gli N elementi dell'array
	}
	
	// vettore di booleani per rappresentare il "bitmask" (sottoinsieme corrente)
	vector<bool> bitmask(N, 0); 
	
	int64_t somma;           // variabile per memorizzare la somma del sottoinsieme corrente
	bool finished = false;   // variabile che indica se abbiamo generato tutte le combinazioni
	
	// ciclo principale: continua finche' non abbiamo generato tutte le combinazioni
	while(!finished){
		somma = 0;
		
		// calcola la somma degli elementi selezionati nel sottoinsieme corrente
		for(int i=0; i<N; i++){
			if(bitmask[i]==1){       // se il bit i e' acceso, includo l'elemento
				somma += a[i]; 
			}
		}
		
		cout << somma << endl;  // stampa la somma del sottoinsieme corrente
		
		// incremento del "bitmask" come se fosse un contatore binario
		for(int i=N-1; i>=0; i--){  // partiamo dall'ultimo bit (bit meno significativo)
			if(bitmask[i] == 0){    // se il bit e' spento
				bitmask[i] = 1;     // accendilo
				break;               // interrompi il ciclo: non serve fare carry
			}
			else{
				bitmask[i] = 0;     // se il bit era acceso, resetta a 0 (carry)
			}
		}
		
		// controllo se abbiamo generato tutte le combinazioni
		finished = true;  // assumiamo che sia l'ultima combinazione
		for (int i=0; i<N; i++){
			if(bitmask[i] != 0){  // se almeno un bit e' acceso
				finished = false;  // non e' l'ultima combinazione
				break;
			} 
		}
